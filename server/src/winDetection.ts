...
function countSuitInAll(suit: string, value: string, allTiles: Tile[]): number {
...
function isBigFourWinds(allTiles: Tile[], melds: Meld[]): boolean {
  ...
  return ['east', 'south', 'west', 'north'].every((w) => countSuitInAll('winds', w, pool) >= 3)
}

function isSmallFourWinds(allTiles: Tile[], melds: Meld[]): boolean {
  const pool = [...allTiles, ...melds.flatMap((m) => m.tiles)]
  const pongs = ['east', 'south', 'west', 'north'].filter((w) => countSuitInAll('winds', w, pool) >= 3)
  const pairs = ['east', 'south', 'west', 'north'].filter((w) => countSuitInAll('winds', w, pool) === 2)
  return pongs.length === 3 && pairs.length === 1
}

function isBigThreeDragons(allTiles: Tile[], melds: Meld[]): boolean {
  const pool = [...allTiles, ...melds.flatMap((m) => m.tiles)]
  return ['red', 'green', 'white'].every((d) => countSuitInAll('dragons', d, pool) >= 3)
}

function isSmallThreeDragons(allTiles: Tile[], melds: Meld[]): boolean {
  const pool = [...allTiles, ...melds.flatMap((m) => m.tiles)]
  const pongs = ['red', 'green', 'white'].filter((d) => countSuitInAll('dragons', d, pool) >= 3)
  const pairs = ['red', 'green', 'white'].filter((d) => countSuitInAll('dragons', d, pool) === 2)
...
function isAllSequences(allTiles: Tile[], melds: Meld[]): boolean {
  if (melds.length > 0) return false // must be fully concealed
  const sorted = sortForBacktrack(allTiles)
  for (let i = 0; i < sorted.length; i++) {
    if (i > 0 && tilesEqual(sorted[i], sorted[i - 1])) continue
    const rest = [...sorted.slice(0, i), ...sorted.slice(i + 1)]
...
export function getWinResult(hand: Tile[], melds: Meld[], newTile: Tile): string | null {
  ...
  // Classify from rarest to most common
...
export function canPong(hand: Tile[], discard: Tile): boolean {
  return hand.filter((t) => tilesEqual(t, discard)).length >= 2
}
...
export function canChow(hand: Tile[], discard: Tile): boolean {
  ...
  const sv = hand.filter((t) => t.suit === discard.suit).map((t) => Number(t.value))
  return (
    (sv.includes(v - 2) && sv.includes(v - 1)) ||
    (sv.includes(v - 1) && sv.includes(v + 1)) ||
    (sv.includes(v + 1) && sv.includes(v + 2))
  )
}

export function canKong(hand: Tile[], discard: Tile): boolean {
  return hand.filter((t) => tilesEqual(t, discard)).length >= 3
}
