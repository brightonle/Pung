/**
 * Client-side win detection — used for UI hints only (e.g. highlighting Win button).
...
export function canWin(hand: Tile[], melds: Meld[], newTile: Tile): boolean {
  ...
  return isStandardWin(allTiles, setsNeeded) || isSpecialWin(allTiles)
}
...
function canFormSets(tiles: Tile[], setsRemaining: number): boolean {
  ...
  // Try chow (3 consecutive, numeric suits only)
  if (isNumericSuit(first.suit)) {
    const v = Number(first.value)
    const idx1 = rest.findIndex(
      (t) => t.suit === first.suit && Number(t.value) === v + 1
    )
    if (idx1 !== -1) {
      const after1 = [...rest.slice(0, idx1), ...rest.slice(idx1 + 1)]
      const idx2 = after1.findIndex(
        (t) => t.suit === first.suit && Number(t.value) === v + 2
      )
      if (idx2 !== -1) {
...
function isSpecialWin(tiles: Tile[]): boolean {
  return isThirteenOrphans(tiles)
}

function isThirteenOrphans(tiles: Tile[]): boolean {
  if (tiles.length !== 14) return false
  const orphans: Array<Pick<Tile, 'suit' | 'value'>> = [
  ...
  for (const orphan of orphans) {
    const idx = remaining.findIndex((t) => tilesEqual(t, orphan as Tile))
    if (idx === -1) return false
    remaining.splice(idx, 1)
  }
  // Last tile must be a duplicate of one of the 13
  return remaining.length === 1 && orphans.some((o) => tilesEqual(remaining[0], o as Tile))
}
